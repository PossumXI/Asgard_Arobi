# PERCILA Demonstration & Accuracy Measurement Guide

## Overview

PERCILA (Precision Engagement & Routing Control with Integrated Learning Architecture) is demonstrable through multiple methods:
...
```powershell
.\scripts\percila_demo.ps1 -Verbose
```
...
|--------|-------------|--------|
| `percila_trajectories_planned_total` | Total trajectories planned | - |
| `percila_trajectory_plan_duration_seconds` | Planning time | < 0.1s |
| `percila_prediction_confidence` | Kalman confidence | > 0.8 |
| `percila_stealth_score_current` | Current stealth score | > 0.7 |
| `percila_detection_events_total` | Detection events | Low |

### Grafana Dashboard Query Examples

```promql
# Average trajectory planning time
rate(percila_trajectory_plan_duration_seconds_sum[5m]) / 
rate(percila_trajectory_plan_duration_seconds_count[5m])

# Prediction confidence distribution
histogram_quantile(0.95, percila_prediction_confidence_bucket)

# Mission success rate
sum(percila_missions_completed_total) / sum(percila_missions_total)
```
...
## Summary: PERCILA Accuracy Claims
...
1. Automated benchmark tests (`Percila/test/benchmark_test.go`)
2. Live API demonstrations (`scripts/percila_demo.ps1`)
...
PERCILA now includes comprehensive physics models for precision payload delivery:
...
PERCILA solves the intercept problem for:
