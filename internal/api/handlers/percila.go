...
type PercilaHandler struct {
	service *services.PercilaService
}

func NewPercilaHandler(service *services.PercilaService) *PercilaHandler {
	return &PercilaHandler{service: service}
}

func (h *PercilaHandler) HandleMissions(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
...
func (h *PercilaHandler) HandlePayloads(w http.ResponseWriter, r *http.Request) {
...
func (h *PercilaHandler) HandleMission(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimPrefix(r.URL.Path, "/api/v1/percila/missions/")
	if id == "" {
