...
func (s *Server) handlePricillaMissions(w http.ResponseWriter, r *http.Request) {
	service := services.NewPricillaService()
	ctx := r.Context()
	...
	case http.MethodGet:
		...
		if err != nil {
			s.writeError(w, http.StatusServiceUnavailable, err.Error(), "PRICILLA_UNAVAILABLE")
			return
		}
...
func (s *Server) handlePricillaMission(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimPrefix(r.URL.Path, "/api/pricilla/missions/")
	if id == "" {
...
func (s *Server) handlePricillaPayloads(w http.ResponseWriter, r *http.Request) {
	service := services.NewPricillaService()
	ctx := r.Context()
