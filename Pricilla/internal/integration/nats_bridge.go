// Package integration provides NATS real-time event integration for PRICILLA.
package integration
...
// ============================================================================
// PRICILLA NATS EVENT TYPES
// ============================================================================

// PricillaEventType represents PRICILLA-specific event types.
type PricillaEventType string
...
const (
	// ASGARD inbound subjects (PRICILLA subscribes to these)
	...
	// PRICILLA outbound subjects (PRICILLA publishes to these)
	SubjectPricillaTrajectory    = "asgard.pricilla.trajectory"
	SubjectPricillaMission       = "asgard.pricilla.mission"
	SubjectPricillaThreatAlert   = "asgard.pricilla.threat_alert"
	SubjectPricillaGuidance      = "asgard.pricilla.guidance"
	SubjectPricillaPayloadStatus = "asgard.pricilla.payload_status"
	SubjectPricillaEvasion       = "asgard.pricilla.evasion"
	SubjectPricillaArrival       = "asgard.pricilla.arrival"
	SubjectPricillaDeviation     = "asgard.pricilla.deviation"
)

// ============================================================================
// PRICILLA EVENT STRUCTURES
// ============================================================================

// PricillaEvent represents a PRICILLA real-time event.
type PricillaEvent struct {
	ID        string                 `json:"id"`
	Type      PricillaEventType      `json:"type"`
...
// NATSBridgeConfig holds configuration for the PRICILLA NATS bridge.
...
// DefaultNATSBridgeConfig returns a default configuration for PRICILLA NATS bridge.
func DefaultNATSBridgeConfig() NATSBridgeConfig {
	return NATSBridgeConfig{
		NATSURL:           "nats://localhost:4222",
		ClusterID:         "asgard-cluster",
		ClientID:          "pricilla-" + generateShortID(),
...
// NATSBridge provides NATS real-time event integration for PRICILLA.
...
// NewNATSBridge creates a new NATS bridge for PRICILLA.
...
func (b *NATSBridge) Connect() error {
	...
	log.Printf("[PRICILLA NATS] Connected to %s", nc.ConnectedUrl())
...
func (b *NATSBridge) Start() error {
	...
	for _, s := range subscriptions {
		...
		if err != nil {
			log.Printf("[PRICILLA NATS] Failed to subscribe to %s: %v", s.subject, err)
			b.recordError(err)
			continue
		}
		b.subscriptions = append(b.subscriptions, sub)
		log.Printf("[PRICILLA NATS] Subscribed to %s", s.subject)
	}

	b.running = true
	log.Println("[PRICILLA NATS] Bridge started successfully")
	return nil
}
...
func (b *NATSBridge) Stop() error {
	...
	for _, sub := range b.subscriptions {
		if err := sub.Unsubscribe(); err != nil {
			log.Printf("[PRICILLA NATS] Error unsubscribing from %s: %v", sub.Subject, err)
		}
	}
	b.subscriptions = nil
	...
	if b.nc != nil {
		if err := b.nc.Drain(); err != nil {
			log.Printf("[PRICILLA NATS] Error draining connection: %v", err)
		}
	}

	b.running = false
	log.Println("[PRICILLA NATS] Bridge stopped")
	return nil
}
...
func (b *NATSBridge) handleThreatMessage(msg *nats.Msg) {
	...
	log.Printf("[PRICILLA NATS] Received threat: %s (severity: %s)", threat.ID, threat.Severity)
	...
	default:
		log.Printf("[PRICILLA NATS] Threat channel full, dropping threat %s", threat.ID)
	}
}
...
func (b *NATSBridge) handleSilenusAlertMessage(msg *nats.Msg) {
	...
	log.Printf("[PRICILLA NATS] Received Silenus alert: %s (type: %s)", alert.ID, alert.Type)
}
...
func (b *NATSBridge) handleMissionMessage(msg *nats.Msg) {
	...
	log.Printf("[PRICILLA NATS] Received mission update: %s (status: %s)", mission.ID, mission.Status)
...
func (b *NATSBridge) handleWeatherMessage(msg *nats.Msg) {
	...
	log.Printf("[PRICILLA NATS] Received weather update for region: %v", weather["region"])
}
...
func (b *NATSBridge) handleNoFlyZoneMessage(msg *nats.Msg) {
	...
	log.Printf("[PRICILLA NATS] Received no-fly zone update: %v", zone["id"])
}
...
func (b *NATSBridge) PublishTrajectoryUpdate(update TrajectoryUpdateEvent) error {
	...
	if err := b.publish(SubjectPricillaTrajectory, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published trajectory update for payload %s", update.PayloadID)
	return nil
}

// PublishThreatAlert publishes a threat alert from PRICILLA.
func (b *NATSBridge) PublishThreatAlert(alert ThreatAlertEvent) error {
	...
	if err := b.publish(SubjectPricillaThreatAlert, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published threat alert for payload %s (threat: %s)", alert.PayloadID, alert.ThreatID)
	return nil
}
...
func (b *NATSBridge) PublishGuidanceCommand(cmd GuidanceCommandEvent) error {
	...
	if err := b.publish(SubjectPricillaGuidance, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published guidance command for payload %s (type: %s)", cmd.PayloadID, cmd.CommandType)
	return nil
}
...
func (b *NATSBridge) PublishPayloadStatus(status PayloadStatusEvent) error {
	...
	if err := b.publish(SubjectPricillaPayloadStatus, data); err != nil {
		return err
	}
...
func (b *NATSBridge) PublishEvasiveManeuver(maneuver EvasiveManeuverEvent) error {
	...
	if err := b.publish(SubjectPricillaEvasion, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published evasive maneuver for payload %s (type: %s)", maneuver.PayloadID, maneuver.ManeuverType)
	return nil
}

// PublishMissionUpdate publishes a PRICILLA mission update.
func (b *NATSBridge) PublishMissionUpdate(missionID string, status string, details map[string]interface{}) error {
	...
	if err := b.publish(SubjectPricillaMission, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published mission update: %s (status: %s)", missionID, status)
	return nil
}
...
func (b *NATSBridge) PublishRouteDeviation(payloadID string, reason string, deviationKm float64, newRoute []Waypoint) error {
	...
	if err := b.publish(SubjectPricillaDeviation, data); err != nil {
		return err
	}

	log.Printf("[PRICILLA NATS] Published route deviation for payload %s (reason: %s, deviation: %.2f km)", payloadID, reason, deviationKm)
	return nil
}

// PublishEvent publishes a generic PRICILLA event.
func (b *NATSBridge) PublishEvent(subject string, event PricillaEvent) error {
	if event.ID == "" {
...
func (b *NATSBridge) onReconnect(nc *nats.Conn) {
	...
	log.Printf("[PRICILLA NATS] Reconnected to %s (reconnect #%d)", nc.ConnectedUrl(), b.stats.Reconnects)

	// Re-establish subscriptions if bridge was running
	if b.running {
		go func() {
			if err := b.Start(); err != nil {
				log.Printf("[PRICILLA NATS] Failed to restart subscriptions after reconnect: %v", err)
			}
		}()
	}
}
...
func (b *NATSBridge) onDisconnect(nc *nats.Conn, err error) {
	if err != nil {
		b.recordError(err)
		log.Printf("[PRICILLA NATS] Disconnected from %s: %v", nc.ConnectedUrl(), err)
	} else {
		log.Printf("[PRICILLA NATS] Disconnected from %s", nc.ConnectedUrl())
	}
}
...
func (b *NATSBridge) onError(nc *nats.Conn, sub *nats.Subscription, err error) {
	...
	log.Printf("[PRICILLA NATS] Error on subject %s: %v", subject, err)
}

func (b *NATSBridge) onClosed(nc *nats.Conn) {
	log.Println("[PRICILLA NATS] Connection closed")
}
